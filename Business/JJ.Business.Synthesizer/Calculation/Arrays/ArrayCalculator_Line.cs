...
{
    internal class ArrayCalculator_Line : ArrayCalculatorBase
    {
        // TODO: extraTimeTickCount 1 should be enough, but it went wrong when I tried a stereo sample.
        // I then temporarily bumped it up to 2, but the source of the problem is unclear.

        private const int EXTRA_TIME_TICK_COUNT = 1;

        public ArrayCalculator_Line(double[,] array, double rate, double minTime = 0)
            : base(array, rate, minTime, EXTRA_TIME_TICK_COUNT)
        { }

        public ArrayCalculator_Line(
            double[,] array,
            double valueBefore,
            double valueAfter,
            double rate,
        ...
        public override double CalculateValue(double time, int channelIndex)
        {
            ...
            int t0 = (int)t;
            int t1 = t0 + 1; // Note that extra time tick count = 1.
