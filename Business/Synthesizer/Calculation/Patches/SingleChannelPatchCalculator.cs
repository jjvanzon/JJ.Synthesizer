...
namespace JJ.Business.Synthesizer.Calculation.Patches
{
    internal class SingleChannelPatchCalculator : PatchCalculatorBase
    {
        ...
        {
            var visitor = new OperatorDtoToCalculatorVisitor_WithCacheOperators(
                calculatorCache,
                curveRepository,
                sampleRepository);

            ToCalculatorResult result = visitor.Execute(dto);
            ...
            _name_To_ResettableOperatorCalculators_Dictionary = result.ResettableOperatorTuples
                                                                      .ToNonUniqueDictionary(
                                                                          x => NameHelper.ToCanonical(x.Name),
                                                                          x => x.OperatorCalculator);
            ...
            VariableInput_OperatorCalculator channelInputCalculator =
                result.Input_OperatorCalculators
        ...
        public override void Reset(double time, string name)
        {
            ...
            // ReSharper disable once InvertIf
            if (_name_To_ResettableOperatorCalculators_Dictionary.TryGetValue(
                canonicalName,
                out IList<OperatorCalculatorBase> calculators))
            {
        ...
        public override void Reset(double time, int position)
        {
            ...
            // ReSharper disable once InvertIf
            if (_position_To_ResettableOperatorCalculators_Dictionary.TryGetValue(
                position,
                out IList<OperatorCalculatorBase> calculators))
            {
                foreach (OperatorCalculatorBase calculator in calculators)
