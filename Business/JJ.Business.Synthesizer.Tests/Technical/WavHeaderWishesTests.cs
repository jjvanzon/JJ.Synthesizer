...
namespace JJ.Business.Synthesizer.Tests.Technical
{
    ...
    public class WavHeaderWishesTests
    {
        ...
        { 
            ...
            TestProp((x, wav) => { FromWavHeader(x.SynthBound.SynthWishes     , wav)                   ; Assert(x.SynthBound.SynthWishes,      test); });
            TestProp((x, wav) => { FromWavHeader(x.SynthBound.FlowNode        , wav)                   ; Assert(x.SynthBound.FlowNode,         test); });
            TestProp((x, wav) => { FromWavHeader(x.SynthBound.ConfigResolver  , wav, synthWishes)      ; Assert(x.SynthBound.ConfigResolver,   test, synthWishes); });
            TestProp((x, wav) => { FromWavHeader(x.TapeBound.Tape             , wav)                   ; Assert(x.TapeBound.Tape,              test); });
            TestProp((x, wav) => { FromWavHeader(x.TapeBound.TapeConfig       , wav)                   ; Assert(x.TapeBound.TapeConfig,        test); });
            TestProp((x, wav) => { FromWavHeader(x.TapeBound.TapeActions      , wav)                   ; Assert(x.TapeBound.TapeActions,       test); });
            TestProp((x, wav) => { FromWavHeader(x.TapeBound.TapeAction       , wav)                   ; Assert(x.TapeBound.TapeAction,        test); });
            TestProp((x, wav) => { FromWavHeader(x.BuffBound.Buff             , wav, courtesy, context); Assert(x.BuffBound.Buff,              test); });
            TestProp((x, wav) => { FromWavHeader(x.BuffBound.AudioFileOutput  , wav, courtesy, context); Assert(x.BuffBound.AudioFileOutput,   test); });
            TestProp((x, wav) => { FromWavHeader(x.Independent.Sample         , wav,           context); Assert(x.Independent.Sample,          test); });
            TestProp((x, wav) => { FromWavHeader(x.Independent.AudioFileInfo  , wav)                   ; Assert(x.Independent.AudioFileInfo,   test); });
            TestProp((x, wav) => { FromWavHeader(x.Independent.AudioInfoWish  , wav)                   ; Assert(x.Independent.AudioInfoWish,   test); });
            TestProp((x, wav) => { WavHeaderWishes.FromWavHeader(x.SynthBound.SynthWishes   , wav)                     ; Assert(x.SynthBound.SynthWishes,    test); });
            TestProp((x, wav) => { WavHeaderWishes.FromWavHeader(x.SynthBound.FlowNode      , wav)                     ; Assert(x.SynthBound.FlowNode,       test); });
            TestProp((x, wav) => { WavHeaderWishesAccessor.FromWavHeader(x.SynthBound.ConfigResolver, wav, synthWishes); Assert(x.SynthBound.ConfigResolver, test, synthWishes); });
            TestProp((x, wav) => { WavHeaderWishes.FromWavHeader(x.TapeBound.Tape           , wav)                     ; Assert(x.TapeBound.Tape,            test); });
            TestProp((x, wav) => { WavHeaderWishes.FromWavHeader(x.TapeBound.TapeConfig     , wav)                     ; Assert(x.TapeBound.TapeConfig,      test); });
            TestProp((x, wav) => { WavHeaderWishes.FromWavHeader(x.TapeBound.TapeActions    , wav)                     ; Assert(x.TapeBound.TapeActions,     test); });
            TestProp((x, wav) => { WavHeaderWishes.FromWavHeader(x.TapeBound.TapeAction     , wav)                     ; Assert(x.TapeBound.TapeAction,      test); });
            TestProp((x, wav) => { WavHeaderWishes.FromWavHeader(x.BuffBound.Buff           , wav,   courtesy, context); Assert(x.BuffBound.Buff,            test); });
            TestProp((x, wav) => { WavHeaderWishes.FromWavHeader(x.BuffBound.AudioFileOutput, wav,   courtesy, context); Assert(x.BuffBound.AudioFileOutput, test); });
            TestProp((x, wav) => { WavHeaderWishes.FromWavHeader(x.Independent.Sample       , wav,             context); Assert(x.Independent.Sample,        test); });
            TestProp((x, wav) => { WavHeaderWishes.FromWavHeader(x.Independent.AudioFileInfo, wav)                     ; Assert(x.Independent.AudioFileInfo, test); });
            TestProp((x, wav) => { WavHeaderWishes.FromWavHeader(x.Independent.AudioInfoWish, wav)                     ; Assert(x.Independent.AudioInfoWish, test); });
            ...
            TestProp((x, wav) => { ApplyTo(wav, x.SynthBound.SynthWishes)                      ; Assert(x.SynthBound.SynthWishes,    test); });
            TestProp((x, wav) => { ApplyTo(wav, x.SynthBound.FlowNode)                         ; Assert(x.SynthBound.FlowNode,       test); });
            TestProp((x, wav) => { ApplyTo(wav, x.SynthBound.ConfigResolver, synthWishes)      ; Assert(x.SynthBound.ConfigResolver, test, synthWishes); });
            TestProp((x, wav) => { ApplyTo(wav, x.TapeBound.Tape)                              ; Assert(x.TapeBound.Tape,            test); });
            TestProp((x, wav) => { ApplyTo(wav, x.TapeBound.TapeConfig)                        ; Assert(x.TapeBound.TapeConfig,      test); });
            TestProp((x, wav) => { ApplyTo(wav, x.TapeBound.TapeActions)                       ; Assert(x.TapeBound.TapeActions,     test); });
            TestProp((x, wav) => { ApplyTo(wav, x.TapeBound.TapeAction)                        ; Assert(x.TapeBound.TapeAction,      test); });
            TestProp((x, wav) => { ApplyTo(wav, x.BuffBound.Buff,            courtesy, context); Assert(x.BuffBound.Buff,            test); });
            TestProp((x, wav) => { ApplyTo(wav, x.BuffBound.AudioFileOutput, courtesy, context); Assert(x.BuffBound.AudioFileOutput, test); });
            TestProp((x, wav) => { ApplyTo(wav, x.Independent.Sample,                  context); Assert(x.Independent.Sample,        test); });
            TestProp((x, wav) => { ApplyTo(wav, x.Independent.AudioFileInfo)                   ; Assert(x.Independent.AudioFileInfo, test); });
            TestProp((x, wav) => { ApplyTo(wav, x.Independent.AudioInfoWish)                   ; Assert(x.Independent.AudioInfoWish, test); });
            TestProp((x, wav) => { WavHeaderWishes.ApplyTo(wav, x.SynthBound.SynthWishes)                        ; Assert(x.SynthBound.SynthWishes,    test); });
            TestProp((x, wav) => { WavHeaderWishes.ApplyTo(wav, x.SynthBound.FlowNode)                           ; Assert(x.SynthBound.FlowNode,       test); });
            TestProp((x, wav) => { WavHeaderWishesAccessor.ApplyTo(wav, x.SynthBound.ConfigResolver, synthWishes); Assert(x.SynthBound.ConfigResolver, test, synthWishes); });
            TestProp((x, wav) => { WavHeaderWishes.ApplyTo(wav, x.TapeBound.Tape)                                ; Assert(x.TapeBound.Tape,            test); });
            TestProp((x, wav) => { WavHeaderWishes.ApplyTo(wav, x.TapeBound.TapeConfig)                          ; Assert(x.TapeBound.TapeConfig,      test); });
            TestProp((x, wav) => { WavHeaderWishes.ApplyTo(wav, x.TapeBound.TapeActions)                         ; Assert(x.TapeBound.TapeActions,     test); });
            TestProp((x, wav) => { WavHeaderWishes.ApplyTo(wav, x.TapeBound.TapeAction)                          ; Assert(x.TapeBound.TapeAction,      test); });
            TestProp((x, wav) => { WavHeaderWishes.ApplyTo(wav, x.BuffBound.Buff,              courtesy, context); Assert(x.BuffBound.Buff,            test); });
            TestProp((x, wav) => { WavHeaderWishes.ApplyTo(wav, x.BuffBound.AudioFileOutput,   courtesy, context); Assert(x.BuffBound.AudioFileOutput, test); });
            TestProp((x, wav) => { WavHeaderWishes.ApplyTo(wav, x.Independent.Sample,                    context); Assert(x.Independent.Sample,        test); });
            TestProp((x, wav) => { WavHeaderWishes.ApplyTo(wav, x.Independent.AudioFileInfo)                     ; Assert(x.Independent.AudioFileInfo, test); });
            TestProp((x, wav) => { WavHeaderWishes.ApplyTo(wav, x.Independent.AudioInfoWish)                     ; Assert(x.Independent.AudioInfoWish, test); });
        }
