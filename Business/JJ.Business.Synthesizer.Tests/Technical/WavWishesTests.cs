...
namespace JJ.Business.Synthesizer.Tests.Technical
{
    ...
    public class WavWishesTests
    {
        ...
        public void WavWishes_FromWish_Test(string caseKey)
        { 
            ...
            TestProp(x => { WavWishes        .FromWish(x.SynthBound.SynthWishes,      info)                   ; Assert(x.SynthBound.SynthWishes,      test); });
            TestProp(x => { WavWishes        .FromWish(x.SynthBound.FlowNode,         info)                   ; Assert(x.SynthBound.FlowNode,         test); });
            TestProp(x => { WavWishesAccessor.FromWish(x.SynthBound.ConfigResolver,   info, synthWishes)      ; Assert(x.SynthBound.ConfigResolver,   test, synthWishes); });
            TestProp(x => { WavWishes        .FromWish(x.TapeBound.Tape,              info)                   ; Assert(x.TapeBound.Tape,              test); });
            TestProp(x => { WavWishes        .FromWish(x.TapeBound.TapeConfig,        info)                   ; Assert(x.TapeBound.TapeConfig,        test); });
            TestProp(x => { WavWishes        .FromWish(x.TapeBound.TapeActions,       info)                   ; Assert(x.TapeBound.TapeActions,       test); });
            TestProp(x => { WavWishes        .FromWish(x.TapeBound.TapeAction,        info)                   ; Assert(x.TapeBound.TapeAction,        test); });
            TestProp(x => { WavWishes        .FromWish(x.BuffBound.Buff,              info, courtesy, context); Assert(x.BuffBound.Buff,              test); });
            TestProp(x => { WavWishes        .FromWish(x.BuffBound.AudioFileOutput,   info, courtesy, context); Assert(x.BuffBound.AudioFileOutput,   test); });
            TestProp(x => { WavWishes        .FromWish(x.Independent.Sample,          info,           context); Assert(x.Independent.Sample,          test); });
            TestProp(x => { WavWishes        .FromWish(x.Independent.AudioFileInfo,   info)                   ; Assert(x.Independent.AudioFileInfo,   test); });
            TestProp(x => { WavWishes        .FromWish(x.Independent.AudioInfoWish,   info)                   ; Assert(x.Independent.AudioInfoWish,   test); });
            TestProp(x => { WavWishes        .ApplyTo(info, x.SynthBound.SynthWishes      )                   ; Assert(x.SynthBound.SynthWishes,      test); });
            TestProp(x => { WavWishes        .ApplyTo(info, x.SynthBound.FlowNode         )                   ; Assert(x.SynthBound.FlowNode,         test); });
            TestProp(x => { WavWishesAccessor.ApplyTo(info, x.SynthBound.ConfigResolver,          synthWishes); Assert(x.SynthBound.ConfigResolver,   test, synthWishes); });
            TestProp(x => { WavWishes        .ApplyTo(info, x.TapeBound.Tape              )                   ; Assert(x.TapeBound.Tape,              test); });
            TestProp(x => { WavWishes        .ApplyTo(info, x.TapeBound.TapeConfig        )                   ; Assert(x.TapeBound.TapeConfig,        test); });
            TestProp(x => { WavWishes        .ApplyTo(info, x.TapeBound.TapeActions       )                   ; Assert(x.TapeBound.TapeActions,       test); });
            TestProp(x => { WavWishes        .ApplyTo(info, x.TapeBound.TapeAction        )                   ; Assert(x.TapeBound.TapeAction,        test); });
            TestProp(x => { WavWishes        .ApplyTo(info, x.BuffBound.Buff,               courtesy, context); Assert(x.BuffBound.Buff,              test); });
            TestProp(x => { WavWishes        .ApplyTo(info, x.BuffBound.AudioFileOutput,    courtesy, context); Assert(x.BuffBound.AudioFileOutput,   test); });
            TestProp(x => { WavWishes        .ApplyTo(info, x.Independent.Sample,                     context); Assert(x.Independent.Sample,          test); });
            TestProp(x => { WavWishes        .ApplyTo(info, x.Independent.AudioFileInfo   )                   ; Assert(x.Independent.AudioFileInfo,   test); });
