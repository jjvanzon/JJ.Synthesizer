...
namespace JJ.Business.Synthesizer.Tests
{
    ...
    public class SynthesizerTests_FM
    {
        ...
        // ReSharper disable once InconsistentNaming
        /// <summary>
        /// Shorthand for OperatorFactor.Value(123) or  x.Value(123). Allows using _[123] instead.
        ...
        private Outlet Composition()
        {
            ...
            (
                x.Multiply(_[fluteVolume], FluteMelody1(portato: 1.1)),
                x.Multiply(_[padVolume], PadChordProgression()),
                x.Multiply(_[tubaVolume], TubaMelody1())
                //x.Multiply(_[rippleBassVolume], RippleBassMelody1())
            );

            var pattern2 = x.Adder
            (
                x.Multiply(_[fluteVolume], FluteMelody2()),
                x.Multiply(_[tubaVolume], TubaMelody2()),
                x.Multiply(_[rippleBassVolume], RippleBassMelody2())
            );

            var composition = x.Adder
            (
                pattern1,
                x.TimeAdd(pattern2, _[BAR * 4])
        ...
        private Outlet FluteMelody1(double portato = 1.3636)
        {
            return x.Adder
            (
                Flute1(_[Frequencies.E4], _[BAR * 0 + BEAT * 0.0], volume: _[0.80], duration: _[2.00 * BEAT * portato]),
                Flute2(_[Frequencies.F4], _[BAR * 0 + BEAT * 1.5], volume: _[0.70], duration: _[2.17 * BEAT * portato]),
                Flute1(_[Frequencies.G4], _[BAR * 0 + BEAT * 3.0], volume: _[0.60], duration: _[1.00 * BEAT * portato]),
                Flute1(_[Frequencies.A4], _[BAR * 1 + BEAT * 0.0], volume: _[0.80], duration: _[2.33 * BEAT * portato]),
                Flute3(_[Frequencies.B4], _[BAR * 1 + BEAT * 1.5], volume: _[0.50], duration: _[1.00 * BEAT * portato]),
                Flute1(_[Frequencies.A4], _[BAR * 1 + BEAT * 3.0], volume: _[0.55], duration: _[1.67 * BEAT * portato]),
                Flute2(_[Frequencies.C4], _[BAR * 2 + BEAT * 0.0], volume: _[1.00], duration: _[2.00 * BEAT * portato]),
                Flute1(_[Frequencies.G4], _[BAR * 2 + BEAT * 1.5], volume: _[0.80], duration: _[2.50 * BEAT * portato])
            );
        }

        private Outlet FluteMelody2() => x.Adder
        (
            Flute1(_[Frequencies.E4], _[BAR * 0 + BEAT * 0.0], volume: _[1.00]),
            Flute2(_[Frequencies.F4], _[BAR * 0 + BEAT * 1.5], volume: _[1.15]),
            Flute2(_[Frequencies.F4], _[BAR * 0 + BEAT * 1.5], volume: _[1.15]),
            Flute3(_[Frequencies.G4], _[BAR * 0 + BEAT * 3.0], volume: _[1.25]),
            Flute4(_[Frequencies.A4], _[BAR * 1 + BEAT * 0.0], volume: _[1.40]),
            Flute3(_[Frequencies.B4], _[BAR * 1 + BEAT * 1.5], volume: _[1.25]),
            Flute2(_[Frequencies.G4], _[BAR * 1 + BEAT * 3.0], volume: _[1.15]),
            Flute4(_[Frequencies.A4], _[BAR * 2 + BEAT * 0.0], volume: _[1.70], duration: _[1.66])
        );
        ...
        private Outlet PadChordProgression()
        {
            ...
            return x.Adder
            (
                x.Sine(_[DEFAULT_AMPLITUDE], StretchCurve(PadPitchCurve1, BAR)),
                x.Sine(_[DEFAULT_AMPLITUDE], StretchCurve(PadPitchCurve2, BAR)),
                x.Sine(_[DEFAULT_AMPLITUDE], StretchCurve(PadPitchCurve3, BAR))
            );
            */
        ...
        private Outlet LowModulation(Outlet freq = null, Outlet delay = null, Outlet volume = null, Outlet duration = null)
        {
            ...
            // FM Algorithm
            Outlet curveDown = StretchCurve(LineDownCurve, x.Multiply(duration, _[1.1]));
            Outlet outlet = FMAroundFreq(
                soundFreq: freq, 
                modSpeed: x.Multiply(freq, _[2.0]), 
                modDepth: x.Multiply(curveDown, _[0.00005]));
        ...
        private Outlet Flute1(Outlet freq = null, Outlet delay = null, Outlet volume = null, Outlet duration = null)
        {
            ...
            // FM Algorithm
            Outlet outlet = FMAround0(x.Divide(freq, _[2]), freq, _[0.005]);
        ...
        private Outlet Flute2(Outlet freq = null, Outlet delay = null, Outlet volume = null, Outlet duration = null)
        {
            ...
            // FM Algorithm
            Outlet outlet = FMAroundFreq(freq, x.Multiply(freq, _[2]), _[0.005]);
            
            // Curve
            outlet = x.Multiply(outlet, StretchCurve(FluteCurve, duration));

            // Equalize Volume
            Outlet equalizedVolume = x.Multiply(volume, _[0.85]);
        ...
        private Outlet Flute3(Outlet freq = null, Outlet delay = null, Outlet volume = null, Outlet duration = null)
        {
            ...
            // FM Algorithm
            Outlet outlet = FMAroundFreq(freq, x.Multiply(freq, _[4]), _[0.005]);

            // Curve
            outlet = x.Multiply(outlet, StretchCurve(FluteCurve, duration));

            // Equalize Volume
            var equalizedVolume = x.Multiply(volume, _[0.8]);
        ...
        private Outlet Flute4(Outlet freq = null, Outlet delay = null, Outlet volume = null, Outlet duration = null)
        {
            ...
            // FM Algorithm
            Outlet outlet = FMAround0(x.Multiply(freq, _[2]), freq, _[0.005]);
            
            // Volume Curve
            outlet = x.Multiply(outlet, StretchCurve(FluteCurve, duration));

            // Equalize Volume
            var equalizedVolume = x.Multiply(volume, _[0.70]);
        ...
        private Outlet Pad(Outlet freq = null, Outlet delay = null, Outlet volume = null, Outlet duration = null)
        {
            ...
            Outlet outlet = x.Add
            (
                FMAroundFreq(freq, x.Multiply(freq, _[2]), x.Multiply(_[0.004], curveDown)),
                FMAroundFreq(freq, x.Multiply(freq, _[3]), x.Multiply(_[0.003], curveDown))
            );
            ...
            // Equalize Loudness
            var equalizedVolume = x.Multiply(volume, _[0.6]);
        ...
        private Outlet ElectricNote(Outlet freq = null, Outlet delay = null, Outlet volume = null, Outlet duration = null)
        {
            ...
            Outlet outlet = x.Add
            (
                FMAroundFreq(freq, x.Multiply(freq, _[1.5]), x.Multiply(_[0.02], StretchCurve(LineDownCurve, duration))),
                FMAroundFreq(freq, x.Multiply(freq, _[2.0]), x.Multiply(_[0.02], StretchCurve(LineDownCurve, duration)))
            );
        ...
        private Outlet Tuba(Outlet freq = null, Outlet delay = null, Outlet volume = null)
        {
            ...
            // FM Algorithm
            var outlet = FMInHertz(x.Multiply(freq, _[2]), freq, _[5]);

            // Stretch Volume Curve (longer when lower)
            const double durationOfA1 = 0.8;
            var freqOfA1 = _[Frequencies.A1];
            var stretch = x.Multiply(_[durationOfA1], x.Power(x.Divide(freqOfA1, freq), _[1.5]));
        ...
        private Outlet RippleBass(Outlet freq = null, Outlet delay = null, Outlet volume = null, Outlet duration = null)
        {
            ...
            // FM algorithm
            Outlet outlet = FMAroundFreq(
                soundFreq: x.Multiply(freq, _[8]), 
                modSpeed: x.Divide(freq, _[2]), 
                modDepth: _[0.005]);
        ...
        private Outlet RippleNote_SharpMetallic(Outlet freq = null)
        {
            ...
            return FMInHertz(freq, x.Divide(freq, _[2]), modDepth: _[10]);
        }
        ...
        private Outlet FMInHertz(Outlet soundFreq, Outlet modSpeed, Outlet modDepth)
        {
            ...
            Outlet sound = x.Sine(_[DEFAULT_AMPLITUDE], x.Add(soundFreq, modulator));
            return sound;
        }
        ...
        {
            Outlet modulator = x.Sine(modDepth, modSpeed);
            Outlet sound = x.Sine(_[DEFAULT_AMPLITUDE], x.Multiply(soundFreq, modulator));
            return sound;
        }
        ...
        {
            Outlet modulator = x.Add(_[1], x.Sine(modDepth, modSpeed));
            Outlet sound = x.Sine(_[DEFAULT_AMPLITUDE], x.Multiply(soundFreq, modulator));
            return sound;
        }
