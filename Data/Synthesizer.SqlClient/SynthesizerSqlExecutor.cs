...
namespace JJ.Data.Synthesizer.SqlClient
{
    public class SynthesizerSqlExecutor
    {
        ...
        public SynthesizerSqlExecutor(ISqlExecutor sqlExecutor) => _sqlExecutor = sqlExecutor ?? throw new NullException(() => sqlExecutor);

        public int GetID() => (int)_sqlExecutor.ExecuteScalar(SqlEnum.GetID);
        ...
        /// </summary>
        public void Sample_TrySetBytes(int id, byte[] bytes) => _sqlExecutor.ExecuteNonQuery(SqlEnum.Sample_TrySetBytes, new { id, bytes });
    }
}