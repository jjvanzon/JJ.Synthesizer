...
{
    public class InterpretedCurveCalculator : ICalculatorWithPosition
    {
        ...
        public double Calculate(double x) => Calculate(x, out bool _);

        public double Calculate(double x, out bool isAtBlockOrStripeBoundary)
        {
            isAtBlockOrStripeBoundary = false;
            ...
            if (node1.GetInterpolationTypeEnum() == InterpolationTypeEnum.Stripe)
            {
                ...
                // Determine is at Stripe boundary
                if (x == halfWay)
                {
                    isAtBlockOrStripeBoundary = true;
                }
            ...
            switch (interpolationTypeEnum)
            {
                ...
                case InterpolationTypeEnum.Block:
                    // Determine is at Block boundary
                    if (x == node0.X)
                    {
                        isAtBlockOrStripeBoundary = true;
                    }
