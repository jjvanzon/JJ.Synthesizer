...
//					// TODO: I want to get the first dimension from the mappings.
//					// Then the dimension value from the PatchCalculator.
//					//calculator.GetValue()
...
//				(DimensionEnum dimensionEnum, string canonicalName, int? position, double dimensionValue)[] dimensionValues = _midiMappingCalculator.CalculateFromMidiControllerValue(midiControllerCode, midiControllerValue);

//				int dimensionValueCount = dimensionValues.Length;
//				for (int dimensionValueIndex = 0; dimensionValueIndex < dimensionValueCount; dimensionValueIndex++)
//				{
//					// Position is not used yet.
//					(DimensionEnum dimensionEnum, string canonicalName, _, double dimensionValue) = dimensionValues[dimensionValueIndex];

//					for (int noteInfoIndex = 0; noteInfoIndex < noteInfoCount; noteInfoIndex++)
//					{
//						NoteInfo noteInfo = noteInfos[noteInfoIndex];
//						int noteIndex = noteInfo.ListIndex;
...
//						if (canonicalName != "")
//						{
//							patchCalculator.SetValue(canonicalName, noteIndex, dimensionValue);
//						}
