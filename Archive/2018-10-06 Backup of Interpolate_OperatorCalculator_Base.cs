...
namespace JJ.Business.Synthesizer.Calculation.Operators
{
	internal abstract class Interpolate_OperatorCalculator_Base : OperatorCalculatorBase_WithChildCalculators
	{
		...
		public sealed override double Calculate()
		{
            ...
            // TODO: While loops may mean performance trouble if _x0 or _x1 are way off.

		    // The if prevents handling both ShiftForward and ShiftBackward at the same time.
		    bool mustShiftForward = MustShiftForward(x);
            if (mustShiftForward)
		    {
		        while (mustShiftForward)
		        {
		            ShiftForward();
		            SetNextSample();
		            Precalculate();

		            mustShiftForward = MustShiftForward(x);
		        }
		    }
		    else
		    {
